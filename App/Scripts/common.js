$(document).ready(function () {
    ...
    $(window).on('load resize', function () {
        if ($(window).innerWidth() < 360) {
            $('.navbar-brand img.logo').attr('src', 'https://fundag-hom.aptuscloud.com.br/solicitacoes/Imagens/logo.png');
        } else {
            $('.navbar-brand img.logo').attr('src', 'https://fundag-hom.aptuscloud.com.br/solicitacoes/Imagens/logo.png');
        }
    });    
...
function CarregarLoader() {
    loading = setTimeout(exibirLoader, 3000);
}

function exibirLoader() {
    document.getElementById("loader").style.display = "none";
    document.getElementById("carregarConteudo").style.display = "block";
}
